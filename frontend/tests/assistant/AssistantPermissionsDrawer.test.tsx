...
describe('AssistantPermissionsDrawer', () => {
  it('loads tool permissions and saves a per-tool override from the assistant', async () => {
    const permissions = {
      ...
      tools: [
        { name: 'get_portfolio', title: 'Truy vấn danh mục', risk: 'read' as const, mode: 'allow' as const, confirmation_required: false },
      ],
    }
    ...
    await screen.findByLabelText('Truy vấn danh mục')
    fireEvent.change(screen.getByLabelText('Truy vấn danh mục'), { target: { value: 'ask' } })
