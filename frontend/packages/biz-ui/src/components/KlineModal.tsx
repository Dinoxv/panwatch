  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className="max-w-5xl">
        <DialogHeader>
          <DialogTitle>{props.title || (symbol ? `Nến: ${symbol}` : 'Nến')}</DialogTitle>
          <DialogDescription>
            {props.description || 'Chuyển nến ngày/tuần/tháng, kèm MA/khối lượng/MACD.'}
          </DialogDescription>
        </DialogHeader>
        {symbol ? (
          <InteractiveKline
          ...
          <div className="text-[12px] text-muted-foreground py-8 text-center">Chưa chọn mã</div>
        )}
      </DialogContent>
    </Dialog>
  )
