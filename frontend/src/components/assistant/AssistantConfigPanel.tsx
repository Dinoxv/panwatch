...
export function AssistantConfigPanel() {
  ...
  useEffect(() => {
    ...
    chatApi.getAssistantConfig()
      ...
      .catch(() => {
        if (active) setError('Không tải được cấu hình ngữ cảnh, xin thử lại sau.')
      })
      .finally(() => {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError('Lưu cấu hình ngữ cảnh thất bại, xin kiểm tra ngưỡng rồi thử lại.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <section aria-label="Cấu hình ngữ cảnh" className="border-t border-border/50 pt-5">
      <div className="mb-3 flex items-start gap-2">
        <Cpu className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
        <div>
          <h3 className="text-[13px] font-semibold text-foreground">Kỹ thuật ngữ cảnh</h3>
          <p className="mt-1 text-[11px] text-muted-foreground">Cấu hình mô hình và hạn mức ngữ cảnh dùng khi tự động nén.</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-6 text-[12px] text-muted-foreground">
          ...
          Đang tải cấu hình ngữ cảnh…
        </div>
      ) : config ? (
        <div className="space-y-3">
          <label className="block text-[11px] text-muted-foreground">
            <span className="mb-1 block">Mô hình nén ngữ cảnh</span>
            <select
              aria-label="Mô hình nén ngữ cảnh"
            ...
            >
              <option value="">Theo mô hình mặc định của hệ thống</option>
          ...
          <label className="block text-[11px] text-muted-foreground">
            <span className="mb-1 block">Nhiệt độ khi nén</span>
            <input
              aria-label="Nhiệt độ khi nén"
              type="number"
              min="0"
              max="2"
              step="0.1"
          ...
          <div className="grid grid-cols-2 gap-2">
            {([
              ['summary_max_tokens', 'Token tối đa cho bản tóm tắt'],
              ['max_tokens', 'Token ngữ cảnh tối đa'],
              ['soft_limit_tokens', 'Ngưỡng tự động nén'],
              ['hard_limit_tokens', 'Trần cứng Token'],
              ['keep_recent_messages', 'Số tin nhắn gần nhất giữ lại'],
          ...
          >
            ...
            {saving ? 'Đang lưu…' : saved ? 'Đã lưu' : 'Lưu cấu hình ngữ cảnh'}
          </button>
        </div>
      ) : null}
    </section>
