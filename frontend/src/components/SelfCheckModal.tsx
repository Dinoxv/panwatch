...
const STATUS_META: Record<RowStatus, {
  ...
  checking: { label: 'Đang kiểm tra', variant: 'secondary', className: 'text-muted-foreground', Icon: Loader2 },
  ok: { label: 'Thông', variant: 'success', className: '', Icon: CheckCircle2 },
  slow: {
    label: 'Chậm',
    variant: 'outline',
  ...
  fail: { label: 'Đứt', variant: 'destructive', className: '', Icon: XCircle },
}
...
/** Gom nhóm theo nhà cung cấp (group), giữ nguyên thứ tự xuất hiện. */
function groupByService(rows: CheckRow[]): Array<[string, CheckRow[]]> {
  const map = new Map<string, CheckRow[]>()
  for (const r of rows) {
    const svc = r.group || 'Chưa phân nhóm'
...
export default function SelfCheckModal({ open, onClose }: SelfCheckModalProps) {
  // Dựng khung nhóm theo danh sách trước (trạng thái đang kiểm tra), mục nào có kết quả thì điền trạng thái của mục đó vào.
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      ...
      setListError(e instanceof Error ? e.message : 'Lấy danh sách tự kiểm tra thất bại')
      setRunning(false)
      return
    }
    if (runId !== runIdRef.current) return
      ...
      while (true) {
        ...
        try {
          ...
          merge(it.key, probed
            ...
            : { status: 'fail', error: 'Không có kết quả kiểm tra trả về', hint: 'Yêu cầu kiểm tra thất bại, hãy thử lại sau' })
        } catch (e) {
          merge(it.key, { status: 'fail', error: e instanceof Error ? e.message : 'Yêu cầu thất bại', hint: 'Yêu cầu kiểm tra thất bại, hãy thử lại sau' })
        }
      }
    }
    await Promise.all(
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Tự kiểm tra hệ thống</DialogTitle>
        </DialogHeader>

        {/* Khối đầu chuyển sắc: thanh tiến độ + tổng / bình thường / bất thường */}
        <div className={`relative overflow-hidden rounded-2xl ${heroGradient} p-4 text-white shadow-lg`}>
          <div className="flex items-center justify-between gap-2">
            <div className="text-[13px] font-semibold">
              {running ? 'Đang kiểm tra…' : finished ? 'Kiểm tra xong' : 'Chuẩn bị kiểm tra'}
            </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-center">
            <div>
              ...
              <div className="mt-1 text-[11px] opacity-80">Tổng</div>
            </div>
            <div>
              <div className="text-[22px] font-bold leading-none tabular-nums">{okCount}</div>
              <div className="mt-1 text-[11px] opacity-80">Bình thường</div>
            </div>
            <div>
              <div className="text-[22px] font-bold leading-none tabular-nums">{failCount}</div>
              <div className="mt-1 text-[11px] opacity-80">Bất thường</div>
            </div>
          </div>
        ...
        {/* Chi tiết từng nhóm: nguồn dữ liệu / mô hình AI (nhà cung cấp → mô hình) / kênh thông báo */}
