...
const AGENT_LABELS: Record<string, string> = {
  ...
  tradingagents: 'TradingAgents chuyên sâu',
}
...
export default function HistoryPage() {
  ...
  const deleteRecord = async (id: number) => {
    if (!confirm('Chắc chắn xóa bản ghi này?')) return
    try {
    ...
    <div className="w-full space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] md:text-[13px] text-muted-foreground">Đọc kiểu báo cáo: mục lục + nội dung</p>
          </div>
          <div className="hidden md:flex px-2.5 py-1 rounded-full bg-background/70 border border-border/50 text-[11px] text-muted-foreground">
            Tổng <span className="font-mono text-foreground/90">{records.length}</span> bản ghi
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Select value={historyKind} onValueChange={(v) => setHistoryKind(v as 'workflow' | 'capability' | 'all')}>
            ...
            <SelectContent>
              <SelectItem value="workflow">Luồng chính</SelectItem>
              <SelectItem value="capability">Tầng năng lực</SelectItem>
              <SelectItem value="all">Tất cả</SelectItem>
            </SelectContent>
          </Select>
          <Select value={selectedAgent} onValueChange={setSelectedAgent}>
            <SelectTrigger className="w-full sm:w-[180px] h-9">
              <SelectValue placeholder="Mọi Agent" />
            </SelectTrigger>
            <SelectContent>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-muted-foreground">Chưa có bản ghi phân tích nào</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[13px] font-medium truncate ${active ? 'text-foreground' : 'text-foreground/90'}`}>{r.title || 'Báo cáo phân tích'}</span>
                    </div>
          ...
          <div className={`md:col-span-7 card p-4 md:p-6 ${mobileView === 'list' ? 'hidden md:block' : ''}`}>
            {selectedRecord ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        // Bản ghi chuyên sâu TradingAgents → trang đọc chi tiết riêng; các agent khác vẫn dùng hộp thoại chi tiết cũ
              ...
              <div className="text-[13px] text-muted-foreground">Xin chọn một bản ghi</div>
            )}
          </div>
        </div>
      )}
      ...
      <Dialog open={!!detailRecord} onOpenChange={open => !open && setDetailRecord(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{detailRecord ? formatTitle(detailRecord) : 'Chi tiết phân tích'}</DialogTitle>
            <DialogDescription>
              {detailRecord && (
          ...
          {detailRecord?.prompt_stats ? (
            <div className="mt-3 rounded-lg border border-border/50 p-3">
              <div className="text-[12px] font-medium mb-1">Thống kê Prompt</div>
          ...
          {detailRecord?.context_payload ? (
            <div className="mt-3 rounded-lg border border-border/50 p-3">
              <div className="text-[12px] font-medium mb-1">Ảnh chụp ngữ cảnh</div>
          ...
          {detailRecord?.news_debug ? (
            <div className="mt-3 rounded-lg border border-border/50 p-3">
              <div className="text-[12px] font-medium mb-1">Chi tiết tin tức đã nạp vào</div>
          ...
          {detailRecord?.prompt_context ? (
            <div className="mt-3 rounded-lg border border-border/50 p-3">
              <div className="text-[12px] font-medium mb-1">Prompt nguyên văn</div>
