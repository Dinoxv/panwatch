...
def test_ngay_lam_viec_la_phien_giao_dich(loaded_calendar):
    """日历已加载时,普通工作日判为交易日。"""
    assert tc.is_trading_day(MarketCode.CN, date(2026, 8, 10)) is True  # Thứ Hai


def test_ngay_nghi_le_khong_phai_phien_giao_dich(loaded_calendar):
...
def test_nhan_ma_thi_truong_dang_chuoi_va_datetime(loaded_calendar):
...
def test_lam_moi_that_bai_khong_nem_loi_van_ha_cap(monkeypatch):
...
def test_lam_moi_bat_dong_bo_khong_chan(monkeypatch):
...
def test_trong_phien_tra_ve_false_ngay_nghi_le(loaded_calendar):
...
def test_trong_phien_tra_ve_true_ngay_giao_dich(loaded_calendar):
    """交易日 10:00 在时段内 → 交易中。"""
    md = MARKETS[MarketCode.CN]
...
def test_ngoai_gio_trong_phien_tra_ve_false(loaded_calendar):
...
def test_cuoi_tuan_khong_gui_ke_hoach_va_tom_tat(monkeypatch):
...
def test_ngay_nghi_le_khong_gui_ke_hoach_va_tom_tat(monkeypatch, loaded_calendar):
...
def test_phien_giao_dich_van_gui_ke_hoach_va_tom_tat(monkeypatch, loaded_calendar):
    """交易日不受守卫影响,通知照常发送。"""
    from src.modules.paper_trading.paper_trading_scheduler import PaperTradingScheduler
...
def test_cuoi_tuan_bo_qua_lam_moi_co_hoi(monkeypatch):
...
def test_co_hoi_van_lam_moi_trong_phien(monkeypatch, loaded_calendar):
    """交易日机会刷新不受守卫影响。"""
    from src.modules.research.context_scheduler import ContextMaintenanceScheduler
...
def test_lam_moi_thu_cong_khong_bi_chan_boi_ngay_nghi(monkeypatch):
    """手动触发是用户显式意图,周末也必须能跑。"""
    from src.modules.research.context_scheduler import ContextMaintenanceScheduler
